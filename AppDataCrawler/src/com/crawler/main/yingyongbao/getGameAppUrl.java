		...
		public void getAppUrl() {
			...
			for (int K = 0; K < categoryIds2.size(); K++) {
				for (int i = 0;; i+=20) {
					...
					try {
						Document doc = Jsoup.connect("http://android.myapp.com/myapp/cate/appList.htm?orgame=2&categoryId="+categoryIds2.get(K)+"&pageSize=20&pageContext="+i).ignoreContentType(true).ignoreHttpErrors(true).timeout(9000).get();
						//记录i，j的值，是为了记录中断的位置，直接从中断的位置开始执行就行
						System.out.println("yingyongbaogame");
